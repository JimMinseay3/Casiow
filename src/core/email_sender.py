...
class EmailSender:
    ...
    def send(self, recipient_info):
        """发送单封邮件（包含所有附件）"""
        try:
            self.logger.info(f"正在向 {recipient_info['email']} 发送邮件，主题: {recipient_info['title']}")
            # 创建邮件对象
            msg = MIMEMultipart()
            ...
            self.logger.info(f"邮件已成功发送至: {recipient_info['email']}")
            return True
    ...
    def send_with_separate_attachments(self, recipient_info):
        ...
        for i, attachment in enumerate(self.config.ATTACHMENTS):
            ...
            self.logger.info(f"发送附件 {i+1}/{total_attachments}: {file_name} 到 {recipient_info['email']}")
            ...
            for attempt in range(3):  # 最多重试3次
                try:
                    ...
                    self.logger.info(f"尝试 {attempt+1}/3 成功 ✔️")
                    success_count += 1
                    break  # 成功后跳出重试循环
                    ...
                    self.logger.error(f"尝试 {attempt+1}/3 SMTP错误 ({error_code}): {e}")
                    
                    # 特定错误码处理
                    if "550" in error_code or "553" in error_code:
                        self.logger.warning("可能是发件人权限问题或附件限制，跳过此附件")
                        fail_count += 1
                        break
                    
                    if attempt < 2:  # 不是最后一次尝试
                        self.logger.info(f"将在 {self.config.SEND_INTERVAL*2} 秒后重试...")
                    ...
                    self.logger.error(f"尝试 {attempt+1}/3 其他错误: {e}")
                    if attempt < 2:
                        self.logger.info(f"将在 {self.config.SEND_INTERVAL*2} 秒后重试...")
                        time.sleep(self.config.SEND_INTERVAL*2)
                    else:
